...
class VisualizerState extends State<Visualizer> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return CustomPaint(painter: VisualizerPainter(_spectrum));
  }
...
class VisualizerPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    LinearGradient gradient = LinearGradient(
        colors: [const Color(0xFF33FF33), const Color(0xFFFF0033)],
    ...
    for (int i = 0; i < COLUMNS_COUNT; i++) {
      ...
      if (_spectrum != null && i < _spectrum.length) {
        volume = _spectrum[i] * step * 1.5 + 2;
        if (volume > size.height) volume = size.height;
      }
      Rect column = Rect.fromLTRB(columnWidth * i, size.height - volume,
          columnWidth * i + columnWidth - 1, size.height);
      canvas.drawRect(
          column,
  ...
  bool shouldRepaint(VisualizerPainter oldDelegate) =>
      true; //oldDelegate._spectrum != _spectrum;
}
